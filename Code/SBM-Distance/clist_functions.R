...
get_initial_values <- function(I, K, Q, hyps)
{
        ...
        list(alpha = alpha, omega = omega, Xi = Xi, omesq = omesq, sigsq = sigsq, 
             zeta = zeta, U = U)
}
...
get_chains_data <- function(I, K, Q, n_sams)
{
        ...
        list(Xi_chain = Xi_chain,
             Eta_chain = Eta_chain)
}

MCMC <- function(Y, K, Q, n_sams, n_burn, n_skip, model, dataset, loc)
{
        I       <- get_I(Y)
...
YPPP <- function(Yna, na_indices, K, Q, n_sams, n_burn, n_skip)
{
        ...
        for (b in 1:B) 
        {
                ...
                Xi          <- sample_Xi(I, K, omega, Eta, Xi, Yna)
                omega       <- sample_omega(K, alpha, Xi)
                sigsq       <- sample_sigsq(K, Q, a_sig, b_sig, U)
                omesq       <- sample_omesq(a_ome, b_ome, zeta)
                tmp         <- sample_alpha(b, n_tun_alpha, del2_alpha, n_alpha, n_burn, K, a_alpha, b_alpha, alpha, omega)
...
incidence_matrix <- function(THETA)
{
        ...
        diag(A) <- 1
        ## plot
        pdf(paste0(loc, "outs/", model, "_", dataset, "_", "incidence_matrix.pdf"), height = 4, width = 4, pointsize = 10)
