...
// [[Rcpp::export]]
List sample_alpha (const double& b, int n_tun_alpha, double del2_alpha, int n_alpha, const int& n_burn, const double& K, const double& a_alpha, const double& b_alpha, double alpha, const vec& omega)
{
        // metropolis step
        double theta_c = log(alpha);
        double theta_p = R::rnorm(theta_c, sqrt(del2_alpha));
        double alpha_p = exp(theta_p);
        while (alpha_p < 0.1) {
                theta_p = R::rnorm(theta_c, sqrt(del2_alpha));
                alpha_p = exp(theta_p);
        }
        if (R::runif(0, 1) < exp((lgamma(alpha_p)-lgamma(alpha)) - K*(lgamma(alpha_p/K)-lgamma(alpha/K)) + (alpha_p-alpha)*accu(log(omega))/K + (a_alpha-1.0)*(log(alpha_p)-log(alpha)) - b_alpha*(alpha_p-alpha) + (theta_p-theta_c))) {
                alpha = alpha_p;
                n_alpha++;
        }
        if (b < n_burn) {
                double mix_rate = n_alpha/b;
