                   ...
                   const double& K, const double& a_alpha, const double& b_alpha, double alpha, const vec& omega)
{
        ...
        if (R::runif(0.0, 1.0) < exp((lgamma(alpha_p)-lgamma(alpha)) - K*(lgamma(alpha_p/K)-lgamma(alpha/K)) + (alpha_p-alpha)*accu(log(omega))/K + (a_alpha-1.0)*(log(alpha_p)-log(alpha)) - b_alpha*(alpha_p-alpha) + (theta_p-theta_c))) {
                alpha = alpha_p;
                n_alpha++;
        }
        if (b < n_burn) {
